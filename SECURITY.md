...
| PII in `audit_log.payload` | Per the 2026-05-03 audit, `redactPayload()` (`src/tool-registry.ts`) walks the args tree and replaces values whose key matches a PII-shaped regex (`/^phone/i`, `/Email$/i`, `/^name$/i`, `/^street/i`, `/^address/i`, `/^city$/i`, `/^zip$/i`, `/^postal/i`, `/^state$/i`, `/^note$/i`, `/^description$/i`, `/^summary$/i`, `/^body$/i`) with type-tagged tokens like `[redacted:str:N]`. Numeric IDs and ISO dates pass through. Recursion depth-capped at 6. |
| Credentials in `audit_log.payload` | The same denylist carries a second, credential-shaped half (`CREDENTIAL_FIELD_PATTERNS`): `secret`, `token`, `api-key`, `authorization`, `password`, `passwd`, `credential`, `bearer`, `cookie`, `session-id`, `sync-key`, `client-secret`, `refresh-token`, `access-token`, `jwt`, `signature`, `private-key`, `salt` — each accepting `-`, `_` or camelCase separators. Rationale: `audit_log` records every tool call, and a log that stores the key material protecting it is a circular defense. Deliberately enumerated rather than a bare `/key/i`, which would eat ordinary fields (`keyName`, `foreignKey`, `keys`) and make audit rows unreadable for no security gain. |
...
| Worker shipping non-SELECT SQL to D1 | `readD1()` (`src/d1.ts`) rejects SQL that doesn't match `^(SELECT\|WITH)\b` before any call leaves the worker. servicetitan-proxy's `/api/sql/read` already enforces SELECT-only server-side; this is defense-in-depth. (The same guard used to be duplicated in `read-router.queryD1`; that module was deleted — it had no live callers — so `readD1` is now the single read path and the single place the guard lives.) |
