  ...
  async handler(env, args, { correlation }) {
    ...
    try {
      const [{ rows }, tableMax] = await Promise.all([
        ...
        fetchTableMax(env, ['dispatch_pro_ratio']),
      ]);
      ...
      // `dispatch_pro_ratio` can't pass as current truth.
      const freshness = stampMirrorFreshness(rows, { table: 'dispatch_pro_ratio', tableMax });
      return {
        count: slice.length,
