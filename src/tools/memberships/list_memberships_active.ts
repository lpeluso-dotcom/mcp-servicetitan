...
export const list_memberships_active: ToolDef<Args> = {
  ...
  description: 'List active memberships. Reads live from ST. D1-first migration tracked as v1.3 follow-up — Phase 1 D1 sync expansion landed 2026-04-28; `memberships` table is populated but tools haven\'t been flipped to D1-first reads yet. Response is trimmed to essential fields; client-side filtered to status=Active since ST statuses param filters on the meaningless active-bool, not the status enum. Default page size 50, max 100 (capped to keep response under the MCP token limit).',
  zodSchema: {
